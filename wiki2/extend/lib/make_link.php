...
EOD;
	...
	function toString()
	{
		...
		$redirect_script = "http://utatanebiyori.uzusionet.com/index.php?u=";
//		return '<a href="' . $this->name . '">' . $this->alias . '</a>';
		$target = (empty($this->redirect)) ? $this->name : $this->redirect.rawurlencode($this->name);
		$add_newwind_tag = open_uri_in_new_window('<a href="' . $target . '"' . $rel . '>' . $this->alias . '</a>', get_class($this));

		$insertpos = mb_strpos($add_newwind_tag, 'http://', 0, mb_detect_encoding($add_newwind_tag));

		return (mb_substr($add_newwind_tag, 0, $insertpos) . $redirect_script . mb_substr($add_newwind_tag, $insertpos));
	}
}
