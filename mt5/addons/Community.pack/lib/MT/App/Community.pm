...
#
# $Id: Community.pm 116991 2009-12-21 02:25:54Z ytakayama $
            ...
            if ( my $file_type = $q->param("type_customfield_$1") ) {
                ...
                if ($asset) {
                    $q->param( "customfield_$1",
                        $asset->as_html( { include => 1, community_pack => 1 } ) );
...
sub _handle_upload {
    ...
    if ( my $allow_exts = MT->config('AssetFileExtensions') ) {
        ...
        ($base, $uploaded_path, $ext) = File::Basename::fileparse($basename, @allowed);
        return $app->error($app->translate('The file([_1]) you uploaded is not allowed.', $basename))
            unless $ext;
    } else {
        ($base, $uploaded_path, $ext) = File::Basename::fileparse($basename, '\.[^\.]*');
    }
