...
class DateUtil{
	...
	function parseTime($str){
		 ...
		 * eq(0,DateUtil::parseTime("00:00:00"));
		 * eq(0,DateUtil::parseTime("0"));
		 * eq(null,DateUtil::parseTime(""));
		 * eq(null,DateUtil::parseTime(null));
		 * eq(null,DateUtil::parseTime("00/00/00"));
		 */
		if(preg_match("/^(\d+):(\d+):(\d+)$/",$str,$match)) $str = (intval($match[1]) * 3600) + (intval($match[2]) * 60) + intval($match[3]);
		if(preg_match("/[^\d]/",$str)) return null;
		return (is_numeric($str)) ? intval($str) : null;
	}
	...
	function formatTime($time){
		 ...
		 * eq("00:00:00",DateUtil::formatTime(0));
		 */
	...
	function weekday($date){
		/***
		 * eq(0,DateUtil::weekday(19050129));
		 * eq(1,DateUtil::weekday(18890211));
		 * eq(2,DateUtil::weekday(20050927));
		 * eq(3,DateUtil::weekday(17890304));
		 * eq(4,DateUtil::weekday(15880721));
		 * eq(5,DateUtil::weekday(18681023));
		 * eq(6,DateUtil::weekday(16001021));
		 * eq(0,DateUtil::weekday("1905-01-29"));
		 * eq(1,DateUtil::weekday("1889/02/11"));
		 */
		if(!is_numeric($date)){
			$date = DateUtil::parseIntDate($date);
		}
		...
		return ($year + V::int($year/4) - V::int($year/100) + V::int($year/400) + V::int((13*$month+8)/5) + $day) % 7;
	}
}
?>