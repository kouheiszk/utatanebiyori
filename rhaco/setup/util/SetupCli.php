		...
		while(true){
			...
			if($in != ""){
				if($in == "." || $in == "q"){
					...
					$this->_init();
				}else if(preg_match("/install[\s]+(.+)[\s]+(.+)$/",$in,$match)){
					$package = trim($match[2]);
					switch(trim($match[1])){
						case "lib":
							print($package." ".(Repository::download("arbo",$package,Rhaco::lib()) ? "installed" : "not found")."\n");
							break;
						case "app":
							print($package." ".(Repository::download("jaja",$package,Rhaco::path()) ? "installed" : "not found")."\n");
							break;
					}
				}else{
