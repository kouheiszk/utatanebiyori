	...
	function install(){
		...
		if($this->isPost()){
			if($this->isVariable("arbo_package")){
				...
				if(Repository::download("arbo",$package,Rhaco::lib())) $message = "installed ".$package;
			}else if($this->isVariable("jaja_package")){
				$package = $this->getVariable("jaja_package");
				$message = $package." not found";
				if(Repository::download("jaja",$package,Rhaco::path())) $message = "installed ".$package;				
			}
		}
